                                    ...
                                    parent
                                        .spawn(NodeBundle {
                                            style: Style {
                                                flex_direction: FlexDirection::Column,
                                                align_items: AlignItems::Center,
                                                margin: UiRect::right(ui::px_p(10.0)),
                                                ..default()
                                            },
                                            ..default()
                                        })
...
const RANGE_BAR_H: f32 = ui::FONT_SIZE * 0.15;
const RANGE_BAR_W: f32 = ui::FONT_SIZE * 1.8;
    ...
    parent
        ...
        .with_children(|parent| {
            parent.spawn((
                NodeBundle {
                    style: Style {
                        width: ui::px_p(range_bar_w.0),
                        height: ui::px_p(RANGE_BAR_H),
                        ..default()
                    },
                    background_color: FG_COLOR.into(),
                    ..default()
                },
                RangeValueBar,
            ));
            parent.spawn((
                NodeBundle {
                    style: Style {
                        width: ui::px_p(range_bar_w.1),
                        height: ui::px_p(RANGE_BAR_H),
                        ..default()
                    },
                    background_color: MUTE_COLOR.into(),
                    ..default()
                },
                RangeBgBar,
            ));
            parent.spawn((
                ...
                .with_style(Style {
                    margin: UiRect::new(ui::px_p(6.0), ui::px_p(0.0), ui::px_p(0.0), ui::px_p(1.0)),
                    ..default()
                }),
                RangeBarText,
            ));
...
fn build_move_testing_panel(parent: &mut ChildBuilder) {
    parent
        .spawn((
            NodeBundle {
                style: Style {
                    width: ui::px_p(MTP_PANEL_SIZE),
                    height: ui::px_p(MTP_PANEL_SIZE),
                    margin: UiRect::top(ui::px_p(6.0)),
                    border: UiRect::all(ui::px_p(0.5)),
                    ..default()
                },
                border_color: SECONDARY_COLOR.into(),
                ..default()
            },
            Interaction::default(),
            MoveTestPanel,
            ...
            parent.spawn((
                NodeBundle {
                    style: Style {
                        ...
                        width: ui::px_p(MTP_BALL_SIZE),
                        height: ui::px_p(MTP_BALL_SIZE),
                        top: ui::px_p(MTP_BALL_POS),
                        left: ui::px_p(MTP_BALL_POS),
                        ..default()
                    },
                    background_color: FG_COLOR.into(),
                    ..default()
...
fn update_slider_display(
    ...
    for child in children {
        for (bar_entity, mut bar_style) in range_value_bar_query.iter_mut() {
            if *child == bar_entity {
                bar_style.width = ui::px_p(range_bar_w.0);
                break;
            }
        }
        for (bar_entity, mut bar_style) in range_bg_bar_query.iter_mut() {
            if *child == bar_entity {
                bar_style.width = ui::px_p(range_bar_w.1);
                break;
            }
        }
